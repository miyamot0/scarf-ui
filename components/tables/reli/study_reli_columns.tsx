    ...
    (question) => {
        ...
        return {
            ...
            cell: ({ row }: { row: Row<StudyObjectPair> }) =>
                style_dynamic(
                    row.original.primary.InternalValidity.Questions.find(
                        (q) => q.QuestionID === question.QuestionID
                    )?.Response,
                    row.original.reliability?.InternalValidity.Questions.find(
                        (q) => q.QuestionID === question.QuestionID
                    )?.Response
                ),
            enableHiding: false,
...
const empirical_cols_2 = ExternalValidityQuestionDefault.Questions.map(
    (question) => {
        ...
        return {
            ...
            cell: ({ row }: { row: Row<StudyObjectPair> }) =>
                style_dynamic(
                    row.original.primary.ExternalValidity.Questions.find(
                        (q) => q.QuestionID === question.QuestionID
                    )?.Response,
                    row.original.reliability?.ExternalValidity.Questions.find(
                        (q) => q.QuestionID === question.QuestionID
                    )?.Response
                ),
            enableHiding: false,
...
const empirical_cols_3 = ReportingQuestionDefault.Questions.map((question) => {
    ...
    return {
        ...
        cell: ({ row }: { row: Row<StudyObjectPair> }) =>
            style_dynamic(
                row.original.primary.Reporting.Questions.find(
                    (q) => q.QuestionID === question.QuestionID
                )?.Response,
                row.original.reliability?.Reporting.Questions.find(
                    (q) => q.QuestionID === question.QuestionID
                )?.Response
            ),
        enableHiding: false,
...
const empirical_cols_4 = OutcomesQuestionDefault.Questions.map((question) => {
    ...
    return {
        ...
        cell: ({ row }: { row: Row<StudyObjectPair> }) =>
            style_dynamic(
                row.original.primary.Outcomes.Questions.find(
                    (q) => q.QuestionID === question.QuestionID
                )?.Response,
                row.original.reliability?.Outcomes.Questions.find(
                    (q) => q.QuestionID === question.QuestionID
                )?.Response
            ),
        enableHiding: false,
...
export const study_columns: ColumnDef<StudyObjectPair>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => (
            ...
            >
                {row.original.primary.StudyYear < 0
                    ? ''
                    : row.original.primary.StudyYear}
            </Badge>
        ),
