...
export function StudiesView() {
    ...
    return (
        <div className="flex flex-col gap-y-4">
            ...
            <p>
                This tab provides a method for adding, coding, and monitoring
                the progress of study coding in the planned data set. By
                default, the app does not perform any local or external saving
                without explicit actions from the user. Data is saved to the
                local browser using the{' '}
                <SaveIcon
                    size={14}
                ...
                />{' '}
                button available in the menu bar. Users are recommended to
                regularly export their data using the{' '}
                <HardDriveDownloadIcon
                    size={14}
                    className="inline border border-input bg-background hover:bg-accent hover:text-accent-foreground h-7 rounded-md px-1 py-1 w-8"
                />{' '}
                button to ensure that their data is not accidentally deleted or
                overwritten.
            </p>

            <p>
                The table below allows for users to add and edit studies as
                necessary. Users can add a new record to the data set using the
                button below the table and individual study records can be
                modified using the{' '}
                <EditIcon
                    size={14}
                    className="inline border border-input bg-background hover:bg-accent hover:text-accent-foreground h-7 rounded-md px-1 py-1 w-8"
